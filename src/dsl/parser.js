...
/**
 * Match a source-map node against a query descriptor. The query only needs
 * the identifying fields for its kind.
 */
function sourceNodeMatches(node, query) {
  if (!node || !query || node.kind !== query.kind) return false;
  switch (query.kind) {
    case 'variable':
    case 'component':
      return node.name === query.name;
    case 'trigger':
      return node.scopeKey === query.scopeKey;
    case 'attribute':
    case 'modulation': {
      if (node.attribute !== query.attribute) return false;
      if (!node.owner || !query.owner || node.owner.kind !== query.owner.kind) return false;
      return node.owner.kind === 'component'
        ? node.owner.name === query.owner.name
        : node.owner.scopeKey === query.owner.scopeKey;
    }
    default:
      return false;
  }
...
class ParseContext {
  ...
  reset() {
    ...
    // line number -> node descriptor (what the line defines/belongs to)
...
class Parser {
  ...
  parse(text) {
    ...
    // Source map: line <-> node lookups for editor features
    // (cursor->panel sync, UI->text write-back, diagnostics)
    const lineMap = this.context.sourceMap;
    const sourceMap = {
      lines: lineMap,
      lineToNode: (line) => lineMap.get(line) || null,
      nodeToLine: (query) => {
        for (const [line, node] of lineMap) {
          if (sourceNodeMatches(node, query)) return line;
        }
        return null;
      },
    };
  ...
  parseLine(line) {
    ...
    // Determine line type and parse; record what node each line produced
    if (this._isVariable(content)) {
      this._parseVariable(content, indent);
      const m = content.match(/^variable\s+([a-zA-Z_][\w]*)/);
      this._recordSource(m ? { kind: 'variable', name: m[1] } : null);
    } else if (this._isTrigger(content)) {
      this._parseTrigger(content, indent);
      const t = this.context.currentTrigger;
      this._recordSource(t ? { kind: 'trigger', type: t.type, scopeKey: t.key } : null);
      ...
      this._recordSource(this._ownerNode('attribute', content));
    } else if (this._isComponent(content)) {
      this._parseComponent(content, indent);
      const c = this.context.currentComponent;
      this._recordSource(c ? { kind: 'component', type: c.type, name: c.name, scopeKey: c.scopeKey } : null);
    } else {
  ...
  /**
   * Record the node descriptor for the line currently being parsed
   */
  _recordSource(node) {
    if (node) {
      this.context.sourceMap.set(this.context.lineNumber, node);
    }
  }

  /**
   * Node descriptor for an attribute/modulation line, tagged with the
   * component or trigger that owns it
   */
  _ownerNode(kind, content) {
    const attribute = content.split(' ')[0];
    const c = this.context.currentComponent;
    if (c) {
      return { kind, attribute, owner: { kind: 'component', type: c.type, name: c.name } };
    }
    const t = this.context.currentTrigger;
    if (t) {
      return { kind, attribute, owner: { kind: 'trigger', type: t.type, scopeKey: t.key } };
    }
    return null;
  }
